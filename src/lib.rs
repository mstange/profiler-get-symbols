...
use samply_api::samply_symbols::{
    ...
    FileLocation, LibraryInfo, OptionallySendFuture,
};
...
extern "C" {
    ...
    /// This type is the file object which JS returned to us from FileAndPathHelper.readFile.
    /// It has the following methods:
    /// - size(): Promise<f64>
    /// - readBytesInto(buffer: Uint8Array, offset: f64): Promise<()>
    /// - close(): Promise<()>
    pub type JsFile;

    #[wasm_bindgen(catch, method, getter)]
    fn size(this: &JsFile) -> Result<f64, JsValue>;

    #[wasm_bindgen(catch, method)]
    fn readBytesInto(this: &JsFile, buffer: js_sys::Uint8Array, offset: f64)
        -> Result<(), JsValue>;

    #[wasm_bindgen(catch, method)]
    fn close(this: &JsFile) -> Result<(), JsValue>;
}
...
/// A wrapper around JsFile which is Send and Sync, and which closes the file on Drop.
///
/// We implement FileByteSource on this. samply-symbols requires FileByteSource to be Send and Sync.
/// In order to satisfy this trait bound, rather than wrapping the JsFile directly, we wrap
/// a u64 handle instead, and store the JsFile in a thread-local registry. That's because JsFile is
/// not Send and Sync - it is a type generated by wasm-bindgen, and hese JS types are not Send and Sync.
///
/// In our WASM context, we are only running on one thread anyway. So the Send/Syncness doesn't
/// really matter. And our thread-local registry is effectively just a global registry.
pub struct JsFileHandle(u64);

use std::cell::RefCell;
thread_local! {
    pub static JS_FILE_REGISTRY: RefCell<HandleRegistry<JsFile>> = RefCell::new(HandleRegistry::new());
}

impl JsFileHandle {
    fn new(value: JsFile) -> Self {
        let id = JS_FILE_REGISTRY.with(|registry| {
            let mut registry = registry.borrow_mut();
            registry.make_handle(value)
        });
        Self(id)
    }
    ...
    fn get(&self) -> JsFile {
        let id = self.0;
        JS_FILE_REGISTRY.with(|registry| {
            let mut registry = registry.borrow_mut();
            JsFile::from(registry.get_value(id).unwrap().clone())
        })
    }
}
...
impl Drop for JsFileHandle {
    fn drop(&mut self) {
        // Close the file.
        let _ = self.get().close();

        // Remove the file from the registry.
        JS_FILE_REGISTRY.with(|registry| {
            let mut registry = registry.borrow_mut();
            registry.remove(self.0).unwrap();
        });
    }
}
...
impl<T> HandleRegistry<T> {
    ...
    pub fn make_handle(&mut self, value: T) -> u64 {
        let id = self.next_id;
        self.next_id += 1;
    ...
    pub fn get_value(&mut self, id: u64) -> Option<&mut T> {
...
impl FileByteSource for JsFileHandle {
    fn read_bytes_into(
        &self,
        buffer: &mut Vec<u8>,
        offset: u64,
...
impl<'h> samply_symbols::FileAndPathHelper<'h> for FileAndPathHelper {
    ...
    > {
        ...
        let future = async move {
            ...
            let file = JsFile::from(file);
            let len = file.size().map_err(JsValueError::from)? as u64;
            let file_handle = JsFileHandle::new(file);
            Ok(FileContentsWithChunkedCaching::new(len, file_handle))
        };
        Box::pin(future)
    }
}
